...
def cut_chip_geometries(
    ...
    generator_window_bounds = img.get_chip_windows(
    ...
    all_chip_dfs = {}
    counter = 0
    for i, (chip_window, chip_transform, chip_poly) in enumerate(
        tqdm(generator_window_bounds)
    ):
        ...
        if not all(chip_df.geometry.is_empty):
            chip_df = chip_df.pipe(
                to_pixelcoords,
                reference_bounds=chip_poly.bounds,
                scale=True,
                ncols=chip_width,
                nrows=chip_height,
            )
