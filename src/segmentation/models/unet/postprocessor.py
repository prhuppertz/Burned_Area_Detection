...
from segmentation.data.postprocessing.watershed import batched_watershed, watershed
...
class Postprocessor:
    ...
    def postprocessing_func(self, pred: np.ndarray) -> np.ndarray:
        ...
        instance_masks = batched_watershed(
            segmentation_masks, dilatation_kernel=self.dilation_kernel
        )
    ...
    def postprocessing_func_single(self, pred: np.ndarray) -> np.ndarray:
        ...
        instance_mask = watershed(
            segmentation_masks, dilation_kernel=self.dilation_kernel
        )
