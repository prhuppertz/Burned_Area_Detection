...
def change_longitude_to_neg(RH_data):
    return RH_data.assign_coords(
        {"longitude": (((RH_data.longitude + 180) % 360) - 180)}
    )
...
def mask_RH_data_to_region(all_nuts_regions, nuts_code, crs, RH_data):
    ...
    region_mask = create_mask_from_geom(
        geom=nuts_code_region.geometry.iloc[0], xrArray=RH_data
    )
...
def get_wf_mask(noon_RH_bound, nearest_coord):
    ...
    noon_RH_clipped = noon_RH_copy.where(
        noon_RH_copy.longitude == nearest_coord[0].values, 1
    ).where(noon_RH_copy.latitude == nearest_coord[1].values, 1)
    return noon_RH_clipped.mask


def open_nuts_shp_with_consistent_crs(crs):
    all_nuts_regions_shp_1m_path = (
        "/data/raw_data/nuts_regions_highres//NUTS_RG_01M_2016_3857.geojson"
    )
...
def create_mask_from_geom(geom, xrArray):
    return regionmask.Regions([geom]).mask(
        xrArray.longitude,
        xrArray.latitude,
        lon_name="longitude",
        lat_name="latitude",
...
def shift_longitude_to_negative_coords(xrArray):
    return xrArray.assign_coords(
        {"longitude": (((xrArray.longitude + 180) % 360) - 180)}
    )


def get_nearest_min_max_coords(WF_shp, noon_RH_bound):
    WF_shp_bounds = WF_shp.geometry.bounds
    nearest_min_long, nearest_min_lat = nearest_latlong(
        WF_shp_bounds[0], WF_shp_bounds[1], noon_RH_bound
    )
    nearest_max_long, nearest_max_lat = nearest_latlong(
        WF_shp_bounds[2], WF_shp_bounds[3], noon_RH_bound
    )
...
def get_bounded_landcover_map(landcover_chunked, bounds):
    ...
    """
    lat_bounded = landcover_chunked.where(
        landcover_chunked.latitude >= bounds.miny.iloc[0], drop=True
    ).where(landcover_chunked.latitude <= bounds.maxy.iloc[0], drop=True)
    landcover_map_bounded = lat_bounded.where(
        lat_bounded.longitude >= bounds.minx.iloc[0], drop=True
    ).where(lat_bounded.longitude <= bounds.maxx.iloc[0], drop=True)
...
def change_dimension_names(landcover_map):
    ...
    landcover_map_changeddims = landcover_map.swap_dims(
