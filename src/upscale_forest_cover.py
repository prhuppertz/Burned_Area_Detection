...
def main(landcover_data_path: str, weather_data_path: str, output_path: str) -> None:
    ...
    landcover_data, weather_data = open_required_data(
        landcover_data_path, weather_data_path
    )
    ...
    fc_group_ids, fc_group_coords = match_forest_pixels_to_closest_weather_pixel(
        coordinate_lists
    )
    del coordinate_lists, weather_data

    logging.info("Compiling and sorting datacube")
    forest_cover_datacube = get_forest_cover_datacube(
        fc_group_ids, fc_group_coords, coordinate_grids["forest_cover"]
    )
    ...
    forest_cover_dataarrays = get_forest_cover_dataarrays_from_cube(
        forest_cover_datacube_sorted
    )
    del forest_cover_datacube_sorted

    forest_cover_grouped_dataset = merge_forest_cover_data(
        forest_cover_dataarrays, forest_cover_binary
    )
    del forest_cover_dataarrays, forest_cover_binary

    logging.info(f"Aggregating DataSet and saving output at {output_path}")
    forest_cover_aggregated_df = aggregate_grouped_forest_cover_data(
        forest_cover_grouped_dataset
    )
    del forest_cover_grouped_dataset
    convert_to_raster_and_write_to_output(forest_cover_aggregated_df, output_path)


def open_required_data(
    landcover_data_path: str, weather_data_path: str
...
def clip_landcover_to_weather_data(
    landcover_data: xr.Dataset, weather_data: xr.Dataset
) -> xr.Dataset:
    """
    ...
    assert np.all(
        np.diff(landcover_data.lon) >= 0
    ), "Landcover most be sorted in ascending order by longitude"
    assert np.all(
        np.diff(landcover_data.lat) <= 0
    ), "Landcover most be sorted in descending order by latitude"
    landcover_clipped = landcover_data.sel(lon=slice(lon_min, lon_max)).sel(
...
def get_forest_cover_from_landcover(landcover_data: xr.Dataset) -> xr.DataArray:
    ...
    forest_cover_binarised_pixels = (
        (landcover_class.values > 30) & (landcover_class.values < 130)
    ).astype(int)

    forest_cover_array = xr.DataArray(
        data=forest_cover_binarised_pixels,
        dims=["lat", "lon"],
...
def get_coordinate_grids(
    ...
    """
    forest_cover_lat_mesh, forest_cover_lon_mesh = np.meshgrid(
        forest_cover_data.lon, forest_cover_data.lat
    )
    weather_lat_mesh, weather_lon_mesh = np.meshgrid(
        weather_data.longitude, weather_data.latitude
    )
    return {
...
def get_coordinate_lists_from_grids(
    ...
    """
    return {
        "forest_cover": np.array(
            [forest_cover[0].flatten(), forest_cover[1].flatten()]
        ).T,
...
def match_forest_pixels_to_closest_weather_pixel(
    ...
    forest_cover_group_coords = coordinate_lists["weather"][
        forest_cover_coord_group_ids
    ]
...
def get_forest_cover_datacube(
    ...
    """
    forest_cover_group_id_array = np.reshape(
        forest_cover_group_ids, forest_cover_grid[0].shape
    )
    forest_cover_group_lon_array = np.reshape(
        forest_cover_group_coords[:, 0], forest_cover_grid[0].shape
    )
    forest_cover_group_lat_array = np.reshape(
        forest_cover_group_coords[:, 1], forest_cover_grid[0].shape
    )
...
def get_forest_cover_dataarrays_from_cube(
    forest_cover_datacube: np.ndarray,
    ...
    name_variable_mapping = {
        ...
        "weather_lat": weather_lat_values,
        "weather_lon": weather_lon_values,
    }
...
def merge_forest_cover_data(
    forest_cover_dataarrays: Dict[str, xr.DataArray], forest_cover_binary: xr.DataArray
) -> xr.Dataset:
    """
    ...
    forest_cover_merged_dataset = xr.merge(
        [forest_cover_group_dataset, forest_cover_binary]
    )

    return forest_cover_merged_dataset


def aggregate_grouped_forest_cover_data(
    forest_cover_grouped_dataset: xr.Dataset,
) -> pd.DataFrame:
    """
    ...
    * first available value for group_lon
    """
    forest_cover_merged_stacked = forest_cover_grouped_dataset.stack(
        {"space": ["lat", "lon"]}
    )
    ...
    forest_cover_df_grouped = forest_cover_df.groupby("group_id").aggregate(
        {
            "forest_cover": pd.Series.mean,
            "group_lat": lambda x: x.iloc[0],
            "group_lon": lambda x: x.iloc[0],
        }
    )
...
def convert_to_raster_and_write_to_output(
    forest_cover_aggregated_df: pd.DataFrame, output_path: str
) -> None:
    """
    ...
    """
    forest_cover_lowres_xarray = (
        forest_cover_aggregated_df.rename(
