...
"""
from patching import (import_shapefile_for_patches, do_the_patching,
                                  store_coco_ground_truth, import_image)
from patchutils.coco import save_gt_overlaid
from docopt import docopt
import os

def main(shapefile, save_path, class_name, scenes_path, scenes, patch_size=128, num_patches=1000):
    ...
    for scene_mgrs in scenes:
        ...
        composite_path = os.path.join(scenes_path, scene_mgrs, '{}.jp2'.format(scene_mgrs))
        ...
        # Load the shapefile
        patch_dfs, patch_windows = import_shapefile_for_patches(shapefile, raster, raster_meta, patch_size,
        ...
        # Save annotations
        store_coco_ground_truth(path_to_store_anno, patch_dfs, patch_size, class_name, scene_mgrs)

        try:
            save_gt_overlaid(os.path.join(path_to_store_anno, 'anno{}.json'.format(scene_mgrs)),
                                  path_to_store_patches, path_to_store_field_vis)
...
if __name__ == '__main__':
    ...
    main(shapefile, save_path, class_name, scenes_path, scenes, patch_size=128, num_patches=1000)
